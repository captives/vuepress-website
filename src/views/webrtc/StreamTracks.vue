            ...
            <el-table-column type="expand">
                <template slot-scope="scope">
                    <el-main class="table-expand">
                        ...
                        <el-row>
                            <h4>Constraints:</h4>
                            <el-col v-for="(item, key) in scope.row.getConstraints()" :key="key" :span="12">{{ key }} :
                                {{ item }}</el-col>
                        </el-row>
                        <el-row>
                            <h4>Settings:</h4>
                            <el-col v-for="(item, key) in scope.row.getSettings()" :key="key" :span="12">{{ key }} :
                                {{ item }}</el-col>
                        </el-row>
                        <el-row>
                            <h4>Capabilities:</h4>
                            <el-col v-for="(item, key) in scope.row.getCapabilities()" :key="key" :span="12">{{ key }} :
                                {{ item }}</el-col>
                        </el-row>
                    </el-main>
                </template>
